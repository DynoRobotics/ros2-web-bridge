...
function fixtime0(thing) {
  if (typeof thing === "object") {
    for (const key in thing) {
      if (!thing.hasOwnProperty(key)) continue; // key is from prototype
      var newkey = key;
      if (key == "secs") newkey = "sec";
      else if(key == "nsecs") newkey = "nanosec";
      thing[newkey] = fixtime0(thing[key])
      if (newkey != key) {
        delete thing[key];
      }
    }
...
function fixtime(thing) {
  if (typeof thing === "object") {
    for (const key in thing) {
      if (!thing.hasOwnProperty(key)) continue; // key is from prototype
      var newkey = key;
      if (key == "sec") newkey = "secs";
      else if(key == "nanosec") newkey = "nsecs";
      thing[newkey] = fixtime(thing[key])
      if (newkey != key) {
        delete thing[key];
      }
    }
...
class Bridge extends EventEmitter {
    ...
    this._registerOpMap('call_service', (command) => {
      ...
      if (client) {
        client.sendRequest(command.args, (response) => {
          ...
          this._ws.send(JSON.stringify(fixtime(serviceResponse)));
        });
      }
    });
    ...
    this._registerOpMap('advertise_service', (command) => {
      ...
      let service = this._resourceProvider.createService(
        ...
        (request, response) => {
          ...
          this._ws.send(JSON.stringify(fixtime(serviceRequest)));
        });
    });
  ...
  _sendSubscriptionResponse(topicName, message) {
    ...
    this._ws.send(JSON.stringify(fixtime(response)));
  }
  ...
  _sendBackOperationStatus(id, level, msg) {
    ...
    this._ws.send(JSON.stringify(fixtime(command)));
  }
